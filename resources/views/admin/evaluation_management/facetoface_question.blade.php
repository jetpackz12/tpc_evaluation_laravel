                                ...
                                <table id="tableActionTools"
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($face_to_face_questions as $face_to_face_question)
                                            <tr>
                                                <td>{{ $counter++ }}</td>
                                                <td>{{ $face_to_face_question->category_name }}</td>
                                                <td>{{ $face_to_face_question->modality }}</td>
                                                <td>{{ $face_to_face_question->question }}</td>
                                                <td>{{ $face_to_face_question->status == 1 ? 'Active' : 'Inactive' }}</td>
                                                <td class="d-flex justify-content-center align-items-center">
                                                    <button class="btn btn-primary mr-1 editWithData"
                                                        data-id="{{ $face_to_face_question->id }}" data-toggle="modal"
                                                        data-target="#modal-edit">
                                                        <i class="fa fa-edit mr-1"></i>
                                                        Edit
                                                    </button>
                                                    @if ($face_to_face_question->status == 1)
                                                        <button class="btn btn-danger editStatus"
                                                            data-id="{{ $face_to_face_question->id }}"
                                                            data-status="{{ $face_to_face_question->status }}"
    ...
    <div class="modal fade" id="modal-add">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form action="{{ route('facetoface_question_store') }}" method="POST" class="postForm">
                    @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" rows="3" id="exampleInputQuestion" name="question" placeholder="Enter Question..."
                                        required></textarea>
                                </div>
                            </div>
                        </div>
    ...
    <div class="modal fade" id="modal-edit">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form action="{{ route('facetoface_question_update') }}" method="POST" class="postForm">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" rows="3" id="e_question" name="question" placeholder="Enter Question..."
                                        required></textarea>
                                </div>
                            </div>
                        </div>
    ...
    <div class="modal fade" id="modal-delete">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form action="{{ route('facetoface_question_status') }}" method="POST" class="postForm">
                    @csrf
                    <div class="modal-body">
    ...
    <script>
        ...
        $(document).ready(function() {
            $('.editWithData').on('click', function() {
                const path = "{{ route('facetoface_question_edit') }}";
